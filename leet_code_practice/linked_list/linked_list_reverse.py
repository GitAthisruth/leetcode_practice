...
def reverseList(head):
        ...
        while current is not None:
            ...
            prev = current
            current = next_node
        return prev # here we returning the final prev value that is 5 , so here we referenced that with the previous val. That way we can reverse the linked list .
...
head.head = reverseList(head.head)
print(head.head.element)
head.print_list()